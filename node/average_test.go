...
func TestAveragePeriodBTC2FCT(t *testing.T) {
	...
	// Min 206422 293471
	for i := uint32(207500); i < 211500; i++ {
		...
		config.PIP10AverageActivation = i + 1
		convert1, err := conversions.Convert(i, 100000000000, BTCprice, BTCavgPrice, FCTprice, FCTavgPrice)
		if err != nil {
			t.Fatal("should not fail")
		}
		config.PIP10AverageActivation = i
		convert2, err := conversions.Convert(i, 100000000000, BTCprice, BTCavgPrice, FCTprice, FCTavgPrice)
		if err != nil {
			t.Fatal("should not fail")
		}
		fctDat.WriteString(fmt.Sprintf("%f\t%f\t%f\t%f\t%f\t%f\n",
