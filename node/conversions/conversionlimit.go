...
// No refund.
func Refund(height uint32, inputAmount, pegYield int64, inputRate, pegRate uint64) int64 {
	maxPEGYield, _ := Convert(height, inputAmount, inputRate, inputRate, pegRate, pegRate)
	refundPEG := maxPEGYield - pegYield
	refund, _ := Convert(height, refundPEG, pegRate, pegRate, inputRate, inputRate)
	return refund
}
